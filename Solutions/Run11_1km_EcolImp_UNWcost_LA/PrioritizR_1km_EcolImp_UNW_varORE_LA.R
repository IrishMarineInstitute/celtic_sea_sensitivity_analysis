...
scenario_name <- "1km_EcolImp_UNWcost_LA" #Each scenario needs a unique name
...
penalty <- round(seq(prelim_penalty[5], prelim_penalty[8], length.out = 9), 5)
...
# Modified to plus 3 here to exclude v. high cost of min orphan solution where bp clearly too high
low_orph_df <- subset(sum_df, orphans == min_orphans | orphans == min_orphans + 2 | orphans == min_orphans + 3)

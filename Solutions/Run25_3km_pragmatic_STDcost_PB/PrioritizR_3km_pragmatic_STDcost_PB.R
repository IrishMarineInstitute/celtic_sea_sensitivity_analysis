...
setwd("H:/mpa_git/Celtic Sea/ProcessedFeatures/FinalFeatures")
...
setwd("H:/mpa_git/Celtic Sea/ProcessedFeatures/CelticSeaFront/split")
...
cellno <- sum(!is.na(getValues(cost_stack_3km[[2]])))
...
scenario_name <- "3km_pragmatic_STDcost_PB" #Each scenario needs a unique name
scenario_group <- "Pragmatic"

date <- as.character(Sys.time())

# Choose cost layer to apply and adjust slice accordingly.  If no costs, use "pu_3km"
names(cost_stack_3km)
cost <- cost_stack_3km[[3]] # Choose cost layer
...
#targets[c(7,8,14,16,25,26,27,28,30,31,32,33,34)] <- 0.32
...
penalty <- round(seq(prelim_penalty[4], prelim_penalty[5], length.out = 9), 5)
penalty
...
for (i in seq_along(solution)) {
  summed_raster <- overlay(solution[[i]], ORE_3km, fun = sum)
...
# NEEDS AMENDING TO TAKE INTO ACCOUNT OVERLAP WITH ORE
# ---------------------------------
...
# best_solution <- solution_layers[[6]]
...
OREmap <- st_read("../../GIS_layers/ORE_poly.shp")
extent <- st_bbox(sa)
...
#   coord_sf(xlim = c(extent[1,1],extent[1,2]), ylim = c(extent[2,1],extent[2,2]))+
...
tracking <- read.csv(file = "Scenarios_PB.csv")    
...
write.csv(tracking, file = "Scenarios_PB.csv", row.names = FALSE)
